...
class ACPClientBase(ABC):
    ...
    async def _handle_message(self, message: Dict):
        """处理消息"""
        logger.debug(f"[ACP] Incoming request: {message.get('method')} id={message.get('id')}")
    ...
    async def _handle_request(self, message: Dict):
        ...
        try:
            ...
            # 终端创建
                ...
                self._terminals[terminal_id] = {"output": "", "exit_code": None}
            ...
            # 终端等待
                ...
                if term_data:
                    while term_data["exit_code"] is None:
...
class ClaudeACPClient(ACPClientBase):
    ...
    async def _initialize(self):
        ...
        # Claude 初始化可能较慢，但根据用户要求由 60s 调回 30s
        try:
...
class ACPConnectionPool:
    ...
    async def acquire(self, session_id: Optional[str] = None):
        ...
        try:
            # Phase 83: 如果指定了 session_id，resume 到它
...
class CLIAgentService:
    ...
    def create_default(cls) -> "CLIAgentService":
        ...
        # 查找 Claude ACP（@zed-industries/claude-code-acp 包）
        # 优先检查本地 node_modules（基于当前文件位置）
        current_dir = os.path.dirname(os.path.abspath(__file__))
        # 假设项目结构: aisop-runtime/src/llm_service/cli_agent_service.py
        # node_modules 在 aisop-runtime/node_modules
        project_root = os.path.abspath(os.path.join(current_dir, "..", "..")) 
        
        local_claude_msg = ""
        local_claude_cmd = os.path.join(project_root, "node_modules", ".bin", "claude-code-acp.cmd")
        
        if os.path.exists(local_claude_cmd):
            claude_cmd = local_claude_cmd
            local_claude_msg = f"Found local claude-code-acp at {local_claude_cmd}"
        else:
            # 尝试使用 CWD 查找 (兼容旧逻辑)
            cwd_claude_cmd = os.path.join(os.getcwd(), "node_modules", ".bin", "claude-code-acp.cmd")
            if os.path.exists(cwd_claude_cmd):
                 claude_cmd = cwd_claude_cmd
                 local_claude_msg = f"Found local claude-code-acp at {cwd_claude_cmd}"
            else:
                 claude_cmd = shutil.which("claude-code-acp") or shutil.which("claude")
                 local_claude_msg = f"Using global/path claude: {claude_cmd}"

        if claude_cmd:
            logger.info(f"[CLIAgentService] {local_claude_msg}")
