...
def _get_claude_pool(bot_data_dir: Optional[str] = None) -> ACPConnectionPool:
    ...
    if cache_key not in _claude_pools:
        import shutil
        # 优先查找本地 node_modules 中的 claude-code-acp（@zed-industries/claude-code-acp 包）
        # 基于当前文件位置向上查找项目根目录
        current_dir = os.path.dirname(os.path.abspath(__file__))
        # 假设项目结构: SoulBot/soulbot/core/llm_service/litellm_acp_provider.py
        project_root = os.path.abspath(os.path.join(current_dir, "..", "..", ".."))

        local_cmd = os.path.join(project_root, "node_modules", ".bin", "claude-code-acp.cmd")
        
        if os.path.exists(local_cmd):
            cmd = local_cmd
            logger.info(f"[ClaudeACPProvider] Found local claude-code-acp at {cmd}")
        else:
            # 尝试 CWD
            cwd_cmd = os.path.join(os.getcwd(), "node_modules", ".bin", "claude-code-acp.cmd")
            if os.path.exists(cwd_cmd):
                cmd = cwd_cmd
                logger.info(f"[ClaudeACPProvider] Found CWD claude-code-acp at {cmd}")
            else:
                # 尝试全局安装的 claude-code-acp
                cmd = shutil.which("claude-code-acp")
                if not cmd:
                    # 最后尝试 claude 命令（@anthropic-ai/claude-code 包）
                    cmd = shutil.which("claude")
                    if not cmd:
                        raise RuntimeError("claude-code-acp not found. Install with: npm install @zed-industries/claude-code-acp")
                    logger.warning(f"[ClaudeACPProvider] Falling back to global/path claude: {cmd}")
                else:
                    logger.info(f"[ClaudeACPProvider] Using global/path claude-code-acp: {cmd}")
...
class ClaudeACPProvider(CustomLLM):
    ...
    async def _acompletion_impl(
        ...
        # Phase 83: 从 optional_params 提取 session_id 和 bot_data_dir
...
class GeminiACPProvider(CustomLLM):
    ...
    async def astreaming(
        ...
        **kwargs
        ...
        bot_data_dir = params.get("bot_data_dir")
        ...
        # 执行流式查询
        async with pool.acquire(session_id=session_id) as (acp_client, _):
            async for text_chunk in acp_client.query_stream(prompt):
                if text_chunk:
                    completion_tokens += len(text_chunk.split())
                    yield {
        ...
        logger.info(f"[GeminiACP] Streaming complete")
